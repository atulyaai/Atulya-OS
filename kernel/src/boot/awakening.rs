...
impl Awakening {
    ...
    pub fn run(&mut self, display: &mut Display) {
        ...
        // Real, calibrated wall-clock timing (see timer.rs) instead of a
        // spin-loop guess — previously this ran in however long 50
        // iterations of an uncalibrated delay(400) took on the host CPU
        // (~1s), not the intended TOTAL_MS.
        const TARGET_FRAME_MS: u32 = 16; // ~60fps target
        let start_ms = crate::timer::now_ms();
        let mut rotation_deg: i32 = 0;
        let mut pulse_osc: u16 = 0;

        loop {
            let frame_start = crate::timer::now_ms();
            let elapsed = frame_start.saturating_sub(start_ms);
            if elapsed > TOTAL_MS {
                break;
            }
            ...
            // Pace to a real frame budget so speed is identical on every
            // machine, instead of "as fast as the CPU can loop".
            let frame_ms = crate::timer::now_ms().saturating_sub(frame_start);
            if frame_ms < TARGET_FRAME_MS {
                crate::timer::delay_ms(TARGET_FRAME_MS - frame_ms);
            }
        }
        ...
        // Brief smooth hold — real 1 second, not a spin-loop guess.
        crate::timer::delay_ms(1_000);
